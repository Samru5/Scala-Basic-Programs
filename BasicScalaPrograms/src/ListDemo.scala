...
object ListDemo extends App {
  ...
  var nums = List(7, 5, 9, 1, 2, "John", true)
  println(nums.reverse) //reverses the list

  println(nums.foreach { i: Any => println(i) }) //foreach loop

  println(nums.drop(2)) //drops first 2 elements of List

  println(nums.take(2)) //prints first 2 elements

  println(nums.drop(2).take(2)) //drops first 2 ele & prints 2 ele from remaining
